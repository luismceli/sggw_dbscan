...
rm(list_tables, wage_h, popul, emplyr, unemplyr, ftpt, activ, edu, gdp, htec_gap, poverty, hrst)
```
...
```{r}
vars_importantes <- select(df_scaled, c(gpg, OLDDEP1, emplygap, UMPLR, pt_f, f_inactiv, gedup_h, gdp_eur, gdp_pps, htec_F , poverty)) %>%
  names()
df_scaled_var1 <- df_scaled %>% select(vars_importantes)
rm (vars_importantes, df_scaled)
```
...
```{r}
# Eliminar variables con baja correlación
df_scaled_var1 <- df_scaled_var1 %>%
  select(-c( gpg ))
```
...
```
#graficar entre el primer y segundo componete su relacion 
...
```{r}
modelo <- dbscan(df_scaled_var1, eps= 2.0 , minPts = 4)
...
```{r}
df_kmeans_final <- merge(df_cluster_kmeans, df_gpg, by = "geo")
df_dbscan_final <- merge(df_cluster_dbscan, df_gpg, by = "geo")

# Finalmente:
aggregate(gpg ~ cluster_kmeans, data = df_kmeans_final, mean)
aggregate(gpg ~ cluster_dbscan, data = df_dbscan_final, mean)
```
```{r}
...
```{r}
# Selección de variables macroeconómicas de interés
vars_importantes <- select(union_df2, c(gpg, OLDDEP1, emplygap, UMPLR, pt_f, f_inactiv, gedup_h, gdp_eur, gdp_pps, htec_F , poverty)) %>%
  names()
...
resumen_real_dbscan <- df_dbscan_real %>%
  filter(cluster_dbscan != 0) %>%  # opcional: eliminar outliers
